...
colors = ['green', 'blue', 'orange', 'blue', 'red', 'purple']
linestyles = ['-', '-', '--', '--', '', ':']
markers = ['+', 'x', 'o', '^', 'v', '.', "<"]
...
def plot(**kwargs):
    ...
    fig.subplots_adjust(left=0.20, top=0.85, right=0.93, bottom=0.20)
    ...
    for x, y in pairwise(cols):
        ...
        extra_args['color'] = colors[i]
        extra_args['ls'] = linestyles[i]
    ...
    if legend:
        ...
        elif legend_pos == 'upper-left-left':
            plt.legend(loc='upper left', bbox_to_anchor=(0.05, 1.25), fontsize='small')
