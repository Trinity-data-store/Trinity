    ...
    */   
    ...
    for (n_leaves_t i = 0; i < n_points; i++){
        ...
        for (dimension_t j = 0; j < DIMENSION; j++){
            if (returned_coordinates->get_coordinate(j) != all_leaf_points[i].get_coordinate(j)){
                return false;
            }
        }    
    }
...
bool test_lookup_close(n_leaves_t n_points, level_t max_depth, level_t trie_depth, preorder_t max_tree_node){
...
bool test_lookup_small_dimension(n_leaves_t n_points, level_t max_depth, level_t trie_depth, preorder_t max_tree_node){
    ...
    */   
    ...
    bitmap::CompactPtrVector tmp_ptr_vect(n_points);
    p_key_to_treeblock_compact = &tmp_ptr_vect;
    create_level_to_num_children(std::vector<level_t>(DIMENSION_SMALL, max_depth), std::vector<level_t>(DIMENSION_SMALL, 0), max_depth);

    auto range = (point_t) 1024;  // A close range
    auto *mdtrie = new md_trie<DIMENSION_SMALL>(max_depth, trie_depth, max_tree_node);
    ...
    for (n_leaves_t i = 0; i < n_points; i++){
        ...
        std::vector<morton_t> node_path_from_primary(max_depth + 1);
        tree_block<DIMENSION_SMALL> *t_ptr = (tree_block<DIMENSION_SMALL> *) (p_key_to_treeblock_compact->At(i));

        point_t parent_symbol_from_primary = t_ptr->get_node_path_primary_key(i, node_path_from_primary);
        node_path_from_primary[max_depth - 1] = parent_symbol_from_primary;

        auto returned_coordinates = t_ptr->node_path_to_coordinates(node_path_from_primary, DIMENSION_SMALL);
