...
public:
  ...
  inline void keep_bits(width_type new_size, bool is_on_data) {
    ...
    if (is_on_data) {
      data_ = (data_type *)realloc(data_,
                                   BITS2BLOCKS(new_size) * sizeof(data_type));
      data_size_ = new_size;
    } else {
      flag_ = (data_type *)realloc(flag_,
                                   BITS2BLOCKS(new_size) * sizeof(data_type));
      flag_size_ = new_size;
    }
  }
  ...
  inline void bulkcopy_backward(pos_type from, pos_type destination,
                                width_type bits, bool is_on_data) {
    while (bits > 64) {
      SetValPos(destination - 64, GetValPos(from - 64, 64, is_on_data), 64,
                is_on_data);
      bits -= 64;
      destination -= 64;
      from -= 64;
  ...
  inline morton_t next_symbol(morton_t symbol, preorder_t node,
    ...
    if (next_block) {
      ...
      if (over_64) {
        return next_symbol(symbol + limit, node, node_pos, end_symbol_range,
                           num_children);
      }
      return end_symbol_range + 1;
    }
    ...
    while (visited < width) {
      if (width - visited > 64) {
        to_dfuds->SetValPos(to_node_pos + visited,
                            GetValPos(from_node_pos + visited, 64, true), 64,
                            true);
        visited += 64;
      } else {
