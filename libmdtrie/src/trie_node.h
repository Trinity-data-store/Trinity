...
public:
  ...
  virtual size_t Serialize(std::ostream &out, level_t num_children,
                           bool is_leaf, bool use_file_offset = false) {
    ...
    if (!is_leaf) {
      ...
      for (uint64_t i = 0; i < num_children; i++) {
        if (!use_file_offset || !get_child(i)) {
          out.write(reinterpret_cast<const char *>(
                        &((trie_node<DIMENSION> **)trie_or_treeblock_ptr_)[i]),
                    sizeof(trie_node<DIMENSION> *));
          out_size += sizeof(trie_node<DIMENSION> *);
        } else {
          out.write(reinterpret_cast<const char *>(
                        &ptr_to_file_offset[(void *)get_child(i)]),
                    sizeof(ptr_to_file_offset[(void *)get_child(i)]));
      ...
      if (!use_file_offset) {
        out.write(reinterpret_cast<const char *>(&trie_or_treeblock_ptr_),
                  sizeof(trie_or_treeblock_ptr_));
        out_size += sizeof(trie_or_treeblock_ptr_);
      } else {
        out.write(reinterpret_cast<const char *>(
                      &ptr_to_file_offset[trie_or_treeblock_ptr_]),
                  sizeof(ptr_to_file_offset[trie_or_treeblock_ptr_]));
        out_size += sizeof(ptr_to_file_offset[trie_or_treeblock_ptr_]);
      }
    }

    out.write(reinterpret_cast<const char *>(&parent_trie_node_),
              sizeof(parent_trie_node_));
    out_size += sizeof(parent_trie_node_);

    out.write(reinterpret_cast<const char *>(&parent_symbol_),
              sizeof(parent_symbol_));
  ...
  virtual size_t Deserialize(std::istream &in, level_t num_children,
                             bool is_leaf, bool use_file_offset = false) {
    ...
    if (!is_leaf) {
      ...
      trie_or_treeblock_ptr_ = static_cast<trie_node<DIMENSION> **>(
          calloc(num_children, sizeof(trie_node<DIMENSION> *)));
      ...
      in.read(reinterpret_cast<char *>(&trie_or_treeblock_ptr_),
              sizeof(trie_or_treeblock_ptr_));
      in_size += sizeof(trie_or_treeblock_ptr_);
    }

    in.read(reinterpret_cast<char *>(&parent_trie_node_),
            sizeof(parent_trie_node_));
    parent_trie_node_ =
        (trie_node<DIMENSION> *)old_ptr_to_new_ptr[(void *)parent_trie_node_];
