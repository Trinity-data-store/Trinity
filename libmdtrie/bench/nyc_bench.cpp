...
void run_bench(){
    ...
    flush_vector_to_file(insertion_latency_vect, results_folder_addr + "nyc_insert_micro" + identification_string);
    infile.close();
    ...
    for (point_t i = 0; i < points_to_insert; i ++) {
        ...
        tree_block<GITHUB_DIMENSION> *t_ptr = (tree_block<GITHUB_DIMENSION> *) (p_key_to_treeblock_compact->At(i));
        morton_t parent_symbol_from_primary = t_ptr->get_node_path_primary_key(i, node_path_from_primary);
        node_path_from_primary[max_depth - 1] = parent_symbol_from_primary;
        t_ptr->node_path_to_coordinates(node_path_from_primary, GITHUB_DIMENSION);
        TimeStamp temp_diff = GetTimestamp() - start;
        cumulative += temp_diff;

        lookup_latency_vect.push_back(temp_diff);
    }
    flush_vector_to_file(lookup_latency_vect, results_folder_addr + "nyc_lookup_micro" + identification_string);
    ...
    */
    ...
    std::string outfile_address = results_folder_addr + "nyc_query_micro" + identification_string;
    ...
    for (int i = 0; i < QUERY_NUM; i ++) {
        ...
        data_point<NYC_DIMENSION> start_range;
        data_point<NYC_DIMENSION> end_range;

        std::string line;
        std::getline(file, line);
        get_query_nyc(line, &start_range, &end_range);

        start = GetTimestamp();
        mdtrie.range_search_trie(&start_range, &end_range, mdtrie.root(), 0, found_points);
        diff = GetTimestamp() - start;
        outfile << "Query " << i << " end to end latency (ms): " << diff / 1000 << ", found points count: " << found_points.size() / NYC_DIMENSION << std::endl;
        found_points.clear();
    }
}
