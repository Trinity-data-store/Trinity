...
void run_bench(){
    ...
    */
    ...
    for (int i = 0; i < QUERY_NUM; i ++) {
        ...
        std::vector<int32_t> found_points;
        data_point<GITHUB_DIMENSION> start_range;
        data_point<GITHUB_DIMENSION> end_range;

        std::string line;
        std::getline(file, line);
        get_query_github(line, &start_range, &end_range);

        start = GetTimestamp();
        mdtrie.range_search_trie(&start_range, &end_range, mdtrie.root(), 0, found_points);
        diff = GetTimestamp() - start;
        outfile << "Query " << i << " end to end latency (ms): " << diff / 1000 << ", found points count: " << found_points.size() / GITHUB_DIMENSION << std::endl;
        found_points.clear();
    }
}
