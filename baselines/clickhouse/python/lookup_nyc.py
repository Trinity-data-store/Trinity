...
def insert_each_worker(worker_idx, total_workers):
    ...
    for i in range(worker_idx, total_points, total_workers):
        ...
        results = client_list[hash_i % num_data_nodes].execute("SELECT * FROM nyc_taxi WHERE pkey = {}".format(total_vect[i]))
        if not results:
...
if int(sys.argv[1]) == 0:
    with open('/proj/trinity-PG0/Trinity/baselines/clickhouse/python/nyc_lookup_throughput.txt', 'a') as f:
