...
def get_random_full_range_one():
    ...
    return '''SELECT * FROM tpch_macro_split WHERE SHIPDATE BETWEEN {} AND {} AND DISCOUNT BETWEEN {} AND {} AND QUANTITY <= {};'''.format(SHIPDATE_start, SHIPDATE_end, DISCOUNT_start, DISCOUNT_end, QUANTITY_end)
...
def get_random_full_range_two():
    ...
    return '''SELECT * FROM tpch_macro_split WHERE ORDERDATE >= {} AND ORDERDATE <= {} AND COMMITDATE < RECEIPTDATE;'''.format(ORDERDATE_start, ORDERDATE_end)
...
def get_random_full_range_three():
    ...
    return '''SELECT * FROM tpch_macro_split WHERE COMMITDATE < RECEIPTDATE AND SHIPDATE < COMMITDATE AND RECEIPTDATE >= {} AND RECEIPTDATE <= {};'''.format(RECEIPTDATE_start, RECEIPTDATE_end)
...
def get_random_full_range_four():
    ...
    return '''SELECT * FROM tpch_macro_split WHERE ORDERDATE <= {} AND SHIPDATE >= {};'''.format(ORDERDATE_end, SHIPDATE_start)
...
def get_random_full_range_five():
    ...
    return '''SELECT * FROM tpch_macro_split WHERE SHIPDATE >= {} AND SHIPDATE <= {};'''.format(SHIPDATE_start, SHIPDATE_end)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "1": # get_random_full_range_one
        template_index = 1
        return_start = 57106873 * (1 - search_range_delta) / test_factor
        return_end = 57106873 * (1 + search_range_delta) / test_factor
    ...
    if sys.argv[1] == "4":
        template_index = 4
        # return_start = 74807478 * (1 - search_range_delta) / test_factor
        # return_end = 74807478 * (1 + search_range_delta) / test_factor
        return_start = 74807478 / 100
        return_end = 74807478 / 10
    if sys.argv[1] == "5":
        template_index = 5
        # return_start = 37417320 * (1 - search_range_delta) / test_factor
        return_start = 37417320 / 100
        # return_end = 37417320 * (1 + search_range_delta) / test_factor
        return_end = 37417320 / 10
