...
def insert_each_worker(worker_idx, total_workers):
    ...
    for i in range(worker_idx, total_points, total_workers):
        ...
        chunk = total_vect[i]
        line_count += 1
        cursor_list[i % 5].execute("INSERT INTO tpch_macro (ID, QUANTITY, EXTENDEDPRICE, DISCOUNT, TAX, SHIPDATE, COMMITDATE, RECEIPTDATE, TOTALPRICE, ORDERDATE) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);", chunk)
...
with open('/proj/trinity-PG0/Trinity/baselines/timescaleDB/python/tpch_throughput.txt', 'a') as f:
