...
public:
  ...
  bool ping(const int32_t dataset_idx) { 
    ...
    {
      if (DIMENSION != 4 || total_points_count != 152806265 || DIMENSION != dimension_to_num_bits.size() || DIMENSION != start_dimension_bits.size() || total_points_count != p_key_to_treeblock_compact->get_num_elements())
        return false;
    }
